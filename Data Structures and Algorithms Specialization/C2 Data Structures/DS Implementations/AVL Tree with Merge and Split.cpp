...
class AVL
{
  public:
	...
	static Node *mergeSplittedAVL(Node *a1, Node *a2, Node *r)
	{
		a1 = rebalance(a1);
		a2 = rebalance(a2);
		r->left = a1;
	...
	static pairAVL splitAt(AVL t, const int &k)
	{
		AVL a1(nullptr), a2(nullptr);
		if (t.root)
		{
			a1.root = t.root->left;
			a2.root = t.root->right;
			if (t.root->key == k)
			{
			...
			{
				pairAVL p = splitAt(a1, k);
				p.second.root = mergeSplittedAVL(p.second.root, t.root->right, t.root);
				return p;
			}
			else if (k > t.root->key)
			{
				pairAVL p = splitAt(a2, k);
				p.first.root = mergeSplittedAVL(a1.root, p.first.root, t.root);
				return p;
			}
		}
		return make_pair(a1, a2);
...
int main()
{
	...
	pairAVL p = AVL::splitAt(r, 11);

	logn("Split Left : ");
	debVect(p.first.getPreorder());
	logn("Split Right : ");
	debVect(p.second.getPreorder());
