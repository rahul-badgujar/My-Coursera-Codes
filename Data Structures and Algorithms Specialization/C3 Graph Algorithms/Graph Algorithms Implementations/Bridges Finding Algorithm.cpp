...
class UnDirGraph
{
  ...
  public:
	...
	void getCutVertices(const Int &u, vector<bool> &visited, vector<Int> &preorder, vector<Int> &low, vector<pair<Int, Int>> &result, vector<Int> &parent)
	{
		static Int time = 0;
		visited[u] = true;
...
int main()
{
	...
	{
		if (!visited[i])
			g.getCutVertices(i, visited, preorder, low, bridges, parent);
	}
	for (auto e : bridges)
	{
		logs(e.first + 1);
